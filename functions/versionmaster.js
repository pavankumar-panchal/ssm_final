...
function formsubmit(command)
{
	...
	if(field.value) { if(validateversions(field.value) == false) { error.innerHTML = errormessage('Product Version format is wrong. Try as in the Example. Eg: v4.15'); field.focus(); return false; } }
	var field = form.releasedate;
	if(!field.value) { error.innerHTML = errormessage('Enter the Release Date'); field.focus(); return false; }
	else
	{
		var passData = "";
		if(command == 'delete')
			...
			passData = 'type=save&lastslno=' + form.lastslno.value + 
						'&s_productgroup=' + form.s_productgroup.value + 
						'&productname=' + form.productname.value + 
						'&productversion=' + form.productversion.value + 
						'&releasedate=' + form.releasedate.value + 
						'&dummy=' + Math.floor(Math.random()*10019200000);
		
		queryString = '../ajax/version-master.php';
...
function getmore(startlimit,slno,showtype)
{
	...
	//alert(passData);
	var queryString = "../ajax/version-master.php";
...
function gridtoform(slno)
{
	...
	ajaxcall1.onreadystatechange = function()
	{
		...
		if(ajaxcall1.readyState == 4)
		{
			...
			form.productname.value = response[1];
			form.productversion.value = response[2];
			form.releasedate.value = response[3];
			form.s_productgroup.value = response[4];
...
function formfilter(command)
{
	...
	var s_productgroup = form.s_productgroup;
	...
	if(command == 'view')
	{
		...
		"&s_productgroup=" + encodeURIComponent(s_productgroup.value) +
		"&selection=" + selection +
	...
	{
		form.action = '../searchreport/versionmaster.php';
		form.target = '_blank';
	    form.submit();
	}
}
...
//Function to select the shortformat and in use of the product acccording to the productname selected---------------
function prdshortformatinuseFunction()
{
	var productname = document.getElementById('productname').value;
	//var shortformatdisplay = document.getElementById('productshortformatdisplay');
	//var productinusedisplay = document.getElementById('productinusedisplay');
	passData = "productname=" + productname  + "&dummy=" + Math.floor(Math.random()*110001787600);
	ajaxcall3 = createajax();
	var queryString = "../ajax/prdshortformatinuse.php";
	...
	ajaxcall3.onreadystatechange = function()
	{
		if(ajaxcall3.readyState == 4)
		{
			...
			//shortformatdisplay.innerHTML = response[0];
			//productinusedisplay.innerHTML = response[1];
		}
	}
	ajaxcall3.send(passData);
}

//Function to validate the version number according to the format [v3.14]-------------------------------------------
function validateversions(versionnumber)
{
	var numericExpression = /^[v][0-9\.\ ]+$/;
	if(versionnumber.match(numericExpression)) return true;
	else return false;
}
function prdgroup(groupname,databasefield1)
{
	if($(databasefield1).attr('checked'))
	{
...
function prdname(elementid,elementname)
{
