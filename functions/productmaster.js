...
function formsubmit(command)
{
	...
	var field = form.shortformat;
	if(!field.value) { error.innerHTML = errormessage('Enter the Short Format'); field.focus(); return false; }
	var field = form.productgroup;
	if(!field.value) { error.innerHTML = errormessage('Enter the Product Group'); field.focus(); return false; }
	var field = form.productinuse;
	if(!field.value) { error.innerHTML = errormessage('Select whether the product is in use'); field.focus(); return false; }
	else
	{
		var passData = "";
		if(command == 'delete')
			...
			passData = 'type=save&lastslno=' + form.lastslno.value + '&productname=' + form.productname.value + '&shortformat=' + form.shortformat.value + '&productgroup=' + form.productgroup.value +  '&productinuse=' + form.productinuse.value + '&dummy=' + Math.floor(Math.random()*10019200000);
		
		queryString = '../ajax/product-master.php';
...
function getmore(startlimit,slno,showtype)
{
	...
	//alert(passData);
	var queryString = "../ajax/product-master.php";
...
function gridtoform(slno)
{
	...
	ajaxcall1.onreadystatechange = function()
	{
		if(ajaxcall1.readyState == 4)
		{
			...
			form.shortformat.value = response[2];
			form.productgroup.value = response[3];
			form.productinuse.value = response[4];
...
function formfilter(command)
{
	...
	{
		form.action = '../searchreport/productmaster.php';
		form.target = '_blank';
	    form.submit();
	}
}
