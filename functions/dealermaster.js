...
function formsubmit(command)
{
	...
	var field = form.dealercompanyname;
	if(!field.value) { error.innerHTML = errormessage('Enter the Dealer Company Name'); field.focus(); return false; }
	var field = form.dealername;
	if(!field.value) { error.innerHTML = errormessage('Enter the Dealer Name'); field.focus(); return false; }
	...
	if(!field.value) { error.innerHTML = errormessage('Enter the Category'); field.focus(); return false; }
	...
	else
	{
		...
		if(command == 'delete')
			...
			passData = 'type=save&lastslno=' + form.lastslno.value + '&dealercompanyname=' + form.dealercompanyname.value + '&dealername=' + form.dealername.value + '&contactnumber=' + form.contactnumber.value + '&emailid=' + form.emailid.value + '&category=' + form.category.value + '&place=' + form.place.value + '&district=' + form.district.value + '&state=' + form.state.value + '&skypeid=' + form.skypeid.value + '&dummy=' + Math.floor(Math.random()*10019200000);
		
		queryString = '../ajax/dealer-master.php';
...
function getmore(startlimit,slno,showtype)
{
	...
	//alert(passData);
	var queryString = "../ajax/dealer-master.php";
...
function gridtoform(slno)
{
	...
	ajaxcall2.onreadystatechange = function()
	{
		...
		if(ajaxcall2.readyState == 4)
		{
			...
			form.dealercompanyname.value = response[1];
			form.dealername.value = response[2];
...
function formfilter(command)
{
	...
	{
		form.action = '../searchreport/dealermaster.php';
		form.target = '_blank';
	    form.submit();
	}
}
