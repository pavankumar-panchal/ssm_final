...
function formsubmit(command)
{
	...
	var field = form.categoryheading;
	if(!field.value) { error.innerHTML = errormessage('Enter the Category Heading'); field.focus(); return false; }
	...
	else
	{
		...
		if(command == 'delete')
			...
			passData = 'type=save&lastslno=' + form.lastslno.value + '&categoryheading=' + form.categoryheading.value + '&remarks=' + form.remarks.value + '&dummy=' + Math.floor(Math.random()*10019200000);
		
		queryString = '../ajax/category-master.php';
...
function getmore(startlimit,slno,showtype)
{
	...
	//alert(passData);
	var queryString = "../ajax/category-master.php";
...
function gridtoform(slno)
{
	...
	ajaxcall2.onreadystatechange = function()
	{
		...
		if(ajaxcall2.readyState == 4)
		{
			...
			form.categoryheading.value = response[1];
...
function formfilter(command)
{
	...
	{
		form.action = '../searchreport/categories.php';
		form.target = '_blank';
	    form.submit();
	}
}
